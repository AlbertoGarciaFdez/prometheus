...
func (n *Notifier) Run() {
	...
	if len(n.opts.AlertmanagerURLs) == 0 {
		log.Warnf("No AlertManagers configured, not dispatching any alerts")
		// Nothing to do, just wait for the termination signal.
		<-n.ctx.Done()
		return
	}
